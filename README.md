...
* **极致性能 (v2.3 优化)**：
    * **智能三层缓存**：
        1. **API 结果缓存**：Cloudflare Edge 直接返回 JSON，耗时 `<10ms`。
        2. **源文件缓存**：Cache API 缓存上游 XML 文件。
        3. **多源内存缓存 (v2.3)**：
            * 支持同时缓存主源和备源数据，防止主备切换时的缓存颠簸。
            * 在 **Docker** 环境下常驻内存，实现 0 CPU 开销的极速查询。
            * **安全熔断**：自动检测 EPG 大小，超过 10MB（约20MB内存）自动禁用内存缓存，防止小内存 Docker 容器崩溃。
    * **极速索引**：重写核心算法，放弃低效的全文正则，改用 **Index Scanning** 技术。内存占用降低 80%，轻松处理 50MB+ 的超大 EPG 文件，杜绝 CPU 超时。
    * **流式传输**：使用 Web Streams API (`pipeThrough`) 处理文件下载。
* **人性化交互**：
...
| `CACHE_TTL` | ❌ 否 | 300 | 源文件及 API 查询结果在 Cloudflare 边缘节点的缓存时间（秒） |
