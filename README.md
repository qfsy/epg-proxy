...
* **极致性能与安全 (v2.4 优化)**：
    * **智能三层缓存**：
        ...
        3. **多源内存缓存**：支持同时缓存主备源数据，防止切换时缓存颠簸。
    * **安全防御**：
        * **网络熔断**：请求上游源时强制 15秒超时，防止 Worker 假死。
        * **内存保护**：自动预检 `Content-Length`，拒绝处理超大文件 (>60MB) 以防止内存溢出。
        * **非特权运行**：Docker 镜像默认使用 `node` 用户运行，提升容器安全性。
    * **极速索引**：改用 **Index Scanning** 技术。内存占用降低 80%，轻松处理大 EPG 文件。
