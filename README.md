...
* **极致性能与安全 (v2.7 核弹级优化)**：
    ...
    * **大内存缓存**：针对 Docker 环境优化，支持 **40MB+** 的大体积 EPG 文本（约 80MB 内存占用）常驻内存，拒绝反复解压，性能提升 10 倍。
    * **智能三层缓存**：
        1. **API 结果缓存**：Cloudflare Edge 直接返回 JSON，耗时 `<10ms`。
        2. **源文件缓存**：Cache API 缓存上游 XML 文件。
        3. **内存热缓存**：解压后的 XML 常驻内存，查询仅需微秒。
    * **安全防御**：
        * **网络熔断**：请求上游源时强制 20秒超时，防止 Worker 假死。
        * **内存保护**：自动预检 `Content-Length`，拒绝处理超大文件 (>150MB) 以防止内存溢出。
...
| `CACHE_TTL` | ❌ 否 | 3600 | 源文件及 API 查询结果在 Cloudflare 边缘节点的缓存时间（秒） |
